  ...
  rules: Object.assign(
    {
      ...
      _assignment: ($) =>
        prec(1, seq($.assignment_lhs, alias("=", $.punctuation))),

      assignment_lhs: ($) => prec(1, choice($.symbol, $.property_expression)),
      ...
      named_context: ($) =>
        prec(
          ...
          seq(
            ...
            optional(
              seq(alias("=", $.punctuation), choice($.symbol, $.string)),
            ),
          ),
        ),

      expression_block: ($) => seq("{", repeat($._expression_component), "}"),
      ...
      parallel_music: ($) =>
        seq(
          ...
          repeat(
            choice(
              $._expression_component,
              alias("\\\\", $.parallel_music_separator),
            ),
          ),
          ">>",
        ),
      ...
      fraction: ($) => token(seq(unsignedInteger(), "/", unsignedInteger())),

      decimal_number: ($) =>
        token(seq(unsignedInteger(), ".", unsignedInteger())),
      ...
      scheme_embedded_lilypond: ($) =>
        seq("#{", repeat($._expression_component), "#}"),
    },
    schemeRules,
  ),
});
